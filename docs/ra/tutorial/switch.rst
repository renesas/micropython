.. _raboard_tutorial_switch:

Switches, callbacks and interrupts
==================================

The raboards have one Reset switch and 1 or 2 User switch(es).
The Reset switch is a hard-reset switch, and if you press it then it 
restarts the raboard from scratch, equivalent to turning the power off 
then back on.

The User switch is for general use, and is controlled via a Switch object.
...
Try holding the User switch down while running the above command.
...
down.  Go ahead and try it: press the User switch and watch the output on

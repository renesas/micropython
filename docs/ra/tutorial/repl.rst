...
interactive MicroPython prompt that you can access on the raboard.  Using
...
To use the REPL, you must connect to the serial USB device on the raboard.
...
for your computer, find the raboard in the list of devices (it should have
a warning sign next to it because it's not working yet), right click on
the raboard device, select Properties, then Install Driver.  You need to
then select the option to find the driver manually (don't use Windows auto update),
navigate to the raboard's USB drive, and select that.  It should then install.
After installing, go back to the Device Manager to find the installed raboard,
...
Now let's try running some MicroPython code directly on the raboard.
...
you have entered the text ``print("hello raboard!")`` and pressed Enter, the output
...
at the MicroPython prompt, which performs a soft reset.  You will see a message something like ::
    ...
    MPY: soft reboot
    Micro Python v1.x on 2021-xx-xx; EK-RA6M2 with RA6M2
...
session, disconnecting whatever program (PuTTY, screen, etc) that you used to connect to the raboard.
